...
const stringArray = (values: string[]): FirestoreValue => ({
  arrayValue: { values: values.map(stringValue) },
});

async function createDocument(path: string, fields: Record<string, FirestoreValue>) {
...
await createDocument('users/seed-crew', {
  name: stringValue('Seed Crew'),
  email: stringValue('crew@seed.test'),
  phone: stringValue('+60000000001'),
  role: stringValue('crew'),
  status: stringValue('verified'),
});

await createDocument('users/seed-operator', {
  name: stringValue('Seed Operator'),
  email: stringValue('operator@seed.test'),
  phone: stringValue('+60000000002'),
  role: stringValue('operator'),
  status: stringValue('approved'),
});

await createDocument('vehicles/seed-vehicle', {
  operatorId: stringValue('seed-operator'),
  registrationNumber: stringValue('SEED-001'),
  status: stringValue('active'),
});

await createDocument('transportRequirements/seed-requirement', {
  crewId: stringValue('seed-crew'),
  assignmentStatus: stringValue('unassigned'),
  tripId: stringValue('seed-trip'),
});

await createDocument('trips/seed-trip', {
  createdBy: stringValue('seed-crew'),
  crewIds: stringArray(['seed-crew']),
  status: stringValue('requested'),
  assignmentStatus: stringValue('unassigned'),
  paymentStatus: stringValue('pending'),
  operatorId: { nullValue: null },
  driverId: { nullValue: null },
  vehicleId: { nullValue: null },
});
